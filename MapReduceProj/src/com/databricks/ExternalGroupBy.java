...
public class ExternalGroupBy {
	...
	public Iterator<Map.Entry<String, List<String>>> groupBy(Iterator<Map.Entry<String, String>> input) {
		...
		System.out.println(files.size() + "     " + Arrays.toString(files.toArray()));

		//multi threading running reduce, keep polling and adding result files into files array till only one file in files array 
		while (files.size() > 1) {
			int reduceRunning = 0;
			...
			for (GroupByReduce task : tasks) {
				try {
					task.join();
					System.out.println(task.getInputFile1() + "    " + task.getInputFile2() + "    " + task.getOutputFilePath());
